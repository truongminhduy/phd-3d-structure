...
Function{
  ...
  If (PML_or_not == 1)
    If (PML_stype == 0)
      ...
      mur[PMLs] = pmltens[] ;  
    Else
      // a_x[] = 1 + ((X[]-D_pml_in/2)/pml_size)^2;
      // a_y[] = 1 + ((Y[]-D_pml_in/2)/pml_size)^2;
      // a_z[] = 1 + ((Z[]-D_pml_in/2)/pml_size)^2;
      // b_x[] = 2*((X[]-D_pml_in/2)/pml_size)^2;
      // b_y[] = 2*((Y[]-D_pml_in/2)/pml_size)^2;
      // b_z[] = 2*((Z[]-D_pml_in/2)/pml_size)^2;
      // sx[Omega_nopml] = 1.;
      // sy[Omega_nopml] = 1.;
      // sz[Omega_nopml] = 1.;
      // sx[PMLxyz] = Complex[a_x[],b_x[]];
      // sy[PMLxyz] = Complex[a_y[],b_y[]];
      // sz[PMLxyz] = Complex[a_z[],b_z[]];
      // sx[PMLxz]  = Complex[a_x[],b_x[]];
      // sy[PMLxz]  = 1.0;
      // sz[PMLxz]  = Complex[a_z[],b_z[]];
      // sx[PMLyz]  = 1.0;
      // sy[PMLyz]  = Complex[a_y[],b_y[]];
      // sz[PMLyz]  = Complex[a_z[],b_z[]];
      // sx[PMLxy]  = Complex[a_x[],b_x[]];
      // sy[PMLxy]  = Complex[a_y[],b_y[]];
      // sz[PMLxy]  = 1.0;
      // sx[PMLx]   = Complex[a_x[],b_x[]];
      // sy[PMLx]   = 1.0;
      // sz[PMLx]   = 1.0;
      // sx[PMLy]   = 1.0;
      // sy[PMLy]   = Complex[a_y[],b_y[]];
      // sz[PMLy]   = 1.0;
      // sx[PMLz]   = 1.0;
      // sy[PMLz]   = 1.0;
      // sz[PMLz]   = Complex[a_z[],b_z[]];
      // pmltens[]  = TensorDiag[ sy[]*sz[]/sx[] , sz[]*sx[]/sy[] , sx[]*sy[]/sz[] ];
      // epsilon[PMLs] = Complex[eps_mil_re,eps_mil_im]   * pmltens[] ;  
      // epsilon1[PMLs] = Complex[eps_mil_re,eps_mil_im] * pmltens[] ;
      // mur[PMLs] = pmltens[] ; 
